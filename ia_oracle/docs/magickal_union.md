...
Conforme descrito a `Dimensão de Inteligência`mantém uma espécie de`Central de Inteligêcia de cujo nível de Observabilidade é de Nível Planetário`,  equipada com telas micro, macro e gigantes, com dashboards sofisticados, `Mostrando as Pulsações do Status atual do Mundo`, disponíveis para todo o sistema (tanto para os módulos autônomos como para os `Usuários Finais`).
